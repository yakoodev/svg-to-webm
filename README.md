...
## Конструктор баннера (`constructor.html`)

Вкладка «Конструктор баннера» — готовые рекламные баннеры, которые меняются без кода. Шаблоны: «Карточка с персонажем (v3d)» и «Аниме-опенинг (манга-панель)» (a). Новые шаблоны добавляются по контракту `docs/TEMPLATE_API.md`.

Что можно менять:

- промокод, сайт, цвета;
- сцены: добавить / удалить / переставить / дублировать; 4 типа — текст, счётчик, скидка + промокод, сайт волной;
- у каждой сцены своя картинка персонажа и смена эмоции «на удар»;
- загрузка своих картинок: PNG с прозрачностью или скрин на зелёном фоне (зелёный вырезается в браузере);
- слово в `{фигурных скобках}` красится акцентом, `{слово|mint}` — выбранным цветом;
- живое превью «на стриме» (баннер 42 % ширины в углу кадра), шахматка, чёрный фон;
- «Скачать WebM» — покадровый рендер с прозрачным фоном прямо в браузере; «Скачать SVG»;
- автосохранение в браузере + «Сохранить/Открыть проект» (JSON с картинками).

Шаблоны: `src/templates/v3d.js`, `src/templates/a.js`; ассеты: `assets/v3d/`, `assets/a/`; шрифты: `assets/fonts/` (Unbounded и Manrope, OFL).

## Экспорт WebM (v8)

Экспорт теперь покадровый через WebCodecs (`src/engine.js`): каждый кадр SVG ставится на точное время
(`pauseAnimations` + `setCurrentTime`, CSS-анимации — через `getAnimations`), значения анимаций берутся у браузера
(`animVal`, computed style), вшитые `@font-face` сохраняются. Кодирование VP9 без пропуска кадров, прозрачность —
вторым VP9-потоком в WebM BlockAdditions (как у libvpx/ffmpeg), OBS и Chrome видят альфу.
Тяжёлый SVG просто считается дольше. Нужен свежий Chrome/Edge; в других браузерах — старая real-time запись.
